...
function convertToLocalCurrency(basePrice, applyTax = true) {
  if (targetCurrencyRate) {
    ...
    return (
      allCurrencies[targetCurrencyKey] +
      numberWithCommas(targetPrice.toFixed(currencyFormat.places))
    );
  } else {
...
function getCurrencyFormat(currencyKey) {
  ...
  return BASE_CURRENCIES.find((currency) => currency.abbr === currencyKey)
    .format;
}
